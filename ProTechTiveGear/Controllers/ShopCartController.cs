...
namespace ProTechTiveGear.Controllers
{
    public class ShopCartController : Controller
    {
        ...
        {
            List<CartEntity> lstCart = Session["ShopCart"] as List<CartEntity>;
            if (lstCart == null)
            {

                lstCart = new List<CartEntity>();
                Session["ShopCart"] = lstCart;
            }
            return lstCart;
        }
        ...
        /// <summary>Đặt hàng: ghi Order + OrderDetail, trừ kho, xóa Session["ShopCart"].</summary>
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult DatHang()
        {
            ...
            foreach (var item in crt)
            {
                ...
                ordt.Totalprice = (decimal)item.TotalPrices;
                db.OrderDetails.Add(ordt);

                var it = db.Items.Find(item.IdItem);
                if (it != null && it.Quantity.HasValue)
                    it.Quantity = it.Quantity - item.Quantity;
            }
            db.SaveChanges();
            Session["ShopCart"] = null;
            return RedirectToAction("OrderConfirmation", "ShopCart");
        }
        ...
        public ActionResult PaymentMoMo()
        {
            ...
            string url = "http://localhost:51494/ShopCart/ReturnUrl/" + or.ID;
        ...
        public ActionResult Newinformation(FormCollection fc)
        {
            ...
            if (Session["usr"] != null)
            {
                ...
                if (temp != null && address != "")
                {
                    ...
                    db.SaveChanges();
                    Session["usr"] = temp;
                    return RedirectToAction("Order", "ShopCart");
