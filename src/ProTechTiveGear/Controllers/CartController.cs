...
namespace ProTechTiveGear.Controllers
{
    public class CartController : Controller
    {
        ...
        private void PersistCart(List<CartEntity> cart)
        {
            int count = 0;
            ...
            {
                Session["Cart"] = cart;
                count = cart.Sum(n => n.Quantity);
            }
            var cookie = new System.Web.HttpCookie("CartCount", count.ToString())
            {
                Path = "/",
                HttpOnly = false,
        ...
        /// <summary>API đếm đơn chưa xử lý — badge trang chủ (đồng bộ Admin).</summary>
        [HttpGet]
        public ActionResult Count()
        {
            ...
            try
            {
                pending = db.Orders.Count(o => o.Status != true);
            }
            catch { }
            return Json(new { count = pending, cart = TotalQuantity() }, JsonRequestBehavior.AllowGet);
        }

        public ActionResult AddtoCart(long id, string strURL)
        {
            bool ajax = string.Equals(Request.Headers["X-Requested-With"], "XMLHttpRequest", StringComparison.OrdinalIgnoreCase)
                || (Request["ajax"] != null && Request["ajax"] == "1");

            var dbItem = db.Items.Find(id);
            if (dbItem == null || !dbItem.Quantity.HasValue || dbItem.Quantity.Value <= 0)
            {
                string msg = "Sản phẩm đã hết hàng";
                if (ajax)
                {
                    int pending = 0;
                    try { pending = db.Orders.Count(o => o.Status != true); } catch { }
                    return Json(new { ok = false, outOfStock = true, message = msg, count = pending }, JsonRequestBehavior.AllowGet);
                }
                ...
                if (!string.IsNullOrWhiteSpace(strURL))
                {
                    try
                    {
                        var uri = new Uri(strURL, UriKind.RelativeOrAbsolute);
                        if (!uri.IsAbsoluteUri) return Redirect(strURL);
            ...
            {
                // Khong cho them qua so luong ton kho
                if (Product.Quantity >= dbItem.Quantity.Value)
                {
                    string msg = "Sản phẩm đã hết hàng";
                    if (ajax)
                    {
            ...
            // Sau khi thêm → mở trang chi tiết đơn + thông tin thanh toán
            if (ajax)
            {
                int pending = 0;
                try { pending = db.Orders.Count(o => o.Status != true); } catch { }
                return Json(new
                {
                    ...
                    count = pending,
                    cart = TotalQuantity(),
                    redirect = Url.Action("Order", "Cart")
        ...
        public ActionResult Cart(bool edit = false)
        {
            List<CartEntity> lstCart = GetCart();
            ...
            // Có hàng → mở form đặt hàng (trừ khi đang sửa số lượng)
            if (!edit && lstCart != null && lstCart.Count > 0)
            {
                return RedirectToAction("Order");
            }
        ...
        public ActionResult EditCart(long id, FormCollection f)
        {
            ...
            PersistCart(lstGiohang);
            return RedirectToAction("Cart", new { edit = true });
        }
        //Xoa Giohang
